...
export const placeBid = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const existingBid = await Bid.findOne({  // Check if the user has already placed a bid 
      "bidder.id": req.user._id,  
      auctionItem: auctionItem._id,
    });
    const existingBidInAuction = auctionItem.bids.find(  // Check if the user has already placed a bid on the auction item
